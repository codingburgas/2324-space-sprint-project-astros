...
void StandardFactCout(int FactCount, int &CelestialBody, string Fact[]) {
	...
	// Based on the given name of the Celestial Body... 
		if (CelestialBody == FactCount) { // ... and the maximum fact count (+5), it prints a corresponding answer: 
			cout << endl;
			centerText("There aren't any more facts to offer about this celestial body\n");
			centerText("Note: The counter will reset and the facts will be repeated all over again.\n");
			...
			for (int i = CelestialBody; i < CelestialBody + 5; i++) {
				centerText(Fact[i]);
			}
			cout << endl;
			CelestialBody += 5;
...
void EarthFact() {
	...
	StandardFactCout(50, Earth, Fact);
}
...
void EarthMoonFact() { // Earth's and Mars' Moon functions are an exception, because they each hold more facts than the rest of the moons and are way less in terms of numbers... 
	// ... with Earth having only one and Mars having 2. 
	string Fact[15] = {
	...
	StandardFactCout(15, EarthMoon, Fact);
}
...
void MarsFact() {
	string Fact[50] = {
		...
		"Mars has a phenomenon known as \"the Medusae Fossae Formation,\" a vast geological formation consisting of soft, easily eroded rock.\n\n",
		"Mars has a day length similar to Earth's, with a rotation period of about 24.6 hours.\n\n",
		"Mars has a year length of about 687 Earth days, or about 1.88 Earth years.\n\n",
		"Mars has a feature known as \"cryptic terrain,\" which refers to areas where the surface appears unusually fractured and jumbled, possibly due to the collapse of underlying lava tubes.\n\n",
		"Mars possesses a region called \"the Chaos Terrain,\" distinct from the aforementioned cryptic terrain, characterized by large blocks of broken, tumbled rocks in a chaotic arrangement. \n\n",
...
void MarsMoonFact() {
	...
	StandardFactCout(25, MarsMoon, Fact);
}
...
void AsteroidBeltFact() {
	...
	StandardFactCout(25, Asteroid, Fact);
}
...
void JupiFact() {
	...
	StandardFactCout(50, Jupi, Fact);
}

void PrintJupiMoonFacts() {
	string JupiMoonFacts[8] = { // It is important to remember that the bigger Celestial bodies such as Jupiter, Saturn, Uranus and Neptune hold more moons that there are facts about 
...
void SatFact() {
	...
	StandardFactCout(50, Sat, Fact);
}

void PrintSatMoonFacts() {
	string SatMoonFacts[12] = {
...
void UranFact() {
	...
	StandardFactCout(50, Uran, Fact);
}

void PrintUranMoonFacts() {
	string UranMoonFacts[15] = {
...
void NepFact() {
	...
	StandardFactCout(50, Nep, Fact);
}

void PrintNepMoonFacts() {
	string NepMoonFacts[9] = {
...
void PluFact() { // While Pluto is not a Planet, it is still one of the major Celestial bodies in the Solar system and deserves a place on the fact sheet 
	...
	StandardFactCout(25, Plu, Fact);
}

void PrintPluMoonFacts() {
	string PluMoonFacts[16] = {
...
void KuiperBeltFact() {
	...
	StandardFactCout(25, Kuiper, Fact);
}

void MoonsFact(string PlanetName) {
	// The MoonsFact() function, depending on the PlanetName that the moon corresponds to, ... 
	if (PlanetName == "The Earth") EarthMoonFact(); // ... either uses one of those two functions (If it is our moon or the moons of Mars we are talking about) ... 
	else if (PlanetName == "Mars") MarsMoonFact();
	else if (PlanetName == "Jupiter") PrintJupiMoonFacts(); // ... or uses the rest of the functions which work differently
	else if (PlanetName == "Saturn") PrintSatMoonFacts();
